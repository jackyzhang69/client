...
class Work extends WebPage {
    ...
    async work_location_duration_actions() {
        for (let i = 0; i < this.data.locations.length; i++) {
            ...
            await this.page.locator("#OPERATING_NAME").fill(location.business_op_name);

            await this.page.locator("#MAIN_BUSINESS_ACTIVITY").fill(location.business_activity);

            await this.page.locator("#MAIN_BUSINESS_SAFETY_HAZARDS").fill(location.safety_concerns);

            try {
                await selectOptionHasSimilarText(this.page, "#WAGE_WORK_LOCATION_JB_ID", location.address);
            } catch (error) {
                print(`The address ${location.address} is not found in the dropdown list, please check it.`, style = "error")
                throw error
            }

            // await this.page.locator("#WAGE_WORK_LOCATION_PROVINCE").selectOption(location.province) // TODO:
    ...
    async duration_number_of_workers_actions() {
        ...
        await this.page.locator("#NUM_TFW_REQUESTED").fill(this.data.number_of_workers)
    }
