...
const getCors = (person) => {
    ...
    if (cors.length === 0) {
        throw new Error("table-cor MUST NOT be empty");
    }
...
const personal_details = (person, role) => {
    ...
    const data = {
        ...
        sex: textIndex(p.sex.trim(), sex_map),
        eye_color: textIndex(p.eye_color.trim(), eye_color),
        ...
        marital_status,
        marital_status_index: textIndex(marital_status.trim(), current_marital_status),
        previous_married,
