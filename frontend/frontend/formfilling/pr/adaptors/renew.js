...
const getAddressHistory = (address) => {
    ...
    for (const addr of address) {
        address_history.push({
            ...
            "country": (new Country(addr.country)).residence_country,
...
const getEducationHistory = (education) => {
    ...
    for (const edu of education) {
        education_history.push({
            ...
            "activity": "study",
            "city": edu.city,
            "country": (new Country(edu.country)).residence_country,
        });
    }
...
const getEmploymentHistory = (employment) => {
    ...
    for (const emp of employment) {
        employment_history.push({
            ...
            "activity": "work",
            "city": emp.city,
            "country": (new Country(emp.country)).residence_country,
        });
    }
...
const renewAdaptor = (data) => {
    ...
    const pr_renewal_data = {
        "application": {
            ...
            "uci": p.uci ? p.uci.match(/\d+/g)?.join("") : "",
        ...
        "personal": {
            ...
            "dob": p.dob.replace(/-/g, '/'),
            "country_of_birth": (new Country(p.country_of_birth)).birth_country,
            "country_of_citizenship": (new Country(p.citizen)).citizen_country,
            "more_than_one_citizenship": p.citizen2 ? true : false,
            "other_citizenships": p.citizen2 ? (new Country(p.citizen2)).citizen_country : "",
            "email": p.email,
