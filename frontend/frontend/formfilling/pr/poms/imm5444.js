...
class Dashboard5444 extends WebPage {
    ...
    async next() {
        ...
        // const editButton = await Promise.race([
        //     getActionableElementInRow(table, 'IMM 5444', 'Start', 'button'),
        //     getActionableElementInRow(table, 'IMM 5444', 'Edit', 'button')
        // ])
        // await editButton.click();
        // await this.page.waitForSelector("h2:has-text('1. Applicant Situation')");
        const table = await this.page.locator('table').first();
        const startButtonPromise = getActionableElementInRow(table, 'IMM 5444', 'Start', 'button');
        const editButtonPromise = getActionableElementInRow(table, 'IMM 5444', 'Edit', 'button');
        const [startButton, editButton] = await Promise.all([startButtonPromise, editButtonPromise]);
        ...
        if (editButton) {
            ...
            throw new Error("Neither 'Start' nor 'Edit' button found for 'IMM 5444' row.");
        }
...
class ApplicantSituation extends WebPage {
    ...
    async make_actions() {
        ...
        await inputDate(this.page, "#date", this.data.date_became_pr);
...
class PersonalDetails extends WebPage {
    ...
    async make_actions() {
        ...
        await this.page.locator("#personalDetailsForm-countryOfBirth").selectOption({ label: this.data.country_of_birth });
        await this.page.locator("#personalDetailsForm-primaryCountryCitizenship").selectOption({ label: this.data.country_of_citizenship });
...
class PersonalHistory extends WebPage {
    ...
    async address_history() {
        ...
        if (btn_clear_table && await btn_clear_table.isEnabled()) {
