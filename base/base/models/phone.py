...
from base.utils.utils import is_consistent
import re
...
class Phone(BaseModel, EqMixin):
    ...
    def check_logic(cls, values):
        required_fields = ["country_code", "number"]
        fields_values = [values.get(f) for f in required_fields]
        missed_fields = [f for f in required_fields if not values.get(f)]

        if not is_consistent(fields_values):
            raise ValueError(f"Required fields missed({', '.join(missed_fields)}) .")

        return values

    @property
    def is_empty(self):
        return False if any([self.country_code, self.number, self.ext]) else True
