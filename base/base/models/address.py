...
class Address(BaseModel):
    ...
    def checkRowCompletion(cls, values):
        """ 
        1. Check for optional rows: if some required fields have value, then others must have too 
        2. Check for required rows: All required fields must exist.
        """
        ...
        if any(all_fields_values) and not all(required_values):
            raise ValueError(
                f"Required fields are missed. ({', '.join(missed_fields)})"
            )
            
        # for must rows
        variable_type=values.get("variable_type")
        display_type=values.get("display_type")
        if variable_type in ["residential_address","mailing_address","business_address"] or variable_type=="working_address" and display_type=="work location 1":
            missed_fields = [
                field for field in required_fields if values.get(field) == None
            ]
