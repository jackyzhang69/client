...
class CELPIP(Language):
    ...
    @property
    def clbs(self):
        return [self.reading, self.writting, self.listening, self.speaking]
...
class TCF(Language):
    ...
    @property
    def clbs(self):
        return self._tcf_2_clbs()
    
def convert(rwls:list,language_type):
    map={
        "IELTS":IELTS,
        "CELPIP":CELPIP,
        "TEF":TEF,
    ...
    if language_type not in map:
        raise ValueError("language type not supported")
    model=map[language_type]
    return model(**score).clbs
