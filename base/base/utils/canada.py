...
""" 
Deprecated version for previous noc app.
"""
area_info = [
    ["Canada", "Canada", 0],
...
""" Check if a province is Canada provice"""
def is_canadian_province(province:str):
    if len(province)==2:
        return True if province.upper() in province_prov.values() else False
    else:
        return True if province.title() in province_prov.keys() else False

""" Check if a post code is a valid Canadian post code """
def is_valid_canadian_postcode(postcode:str):
    # Postal codes are always in the format A1A 1A1
    if not re.match(r'[A-Z]\d[A-Z] \d[A-Z]\d', postcode.upper()):
        return False
