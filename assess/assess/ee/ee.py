...
class EEScoring(ScoringBase):
    ...
    def convert_clbs(cls, values):
        ...
        if values['second_clbs']:
            values['second_clbs'] = convert(values['second_clbs'],values['second_clbs_type'])
        if values['spouse_clbs']:
            values['spouse_clbs'] = convert(values['spouse_clbs'],values['spouse_clbs_type'])
        return values
    ...
    def fsw_point(self):
        factors = {
            ...
            "spouse_worked": True if self.spouse_canadian_work_experience > 0 else False,
