...
def getRangePoint(points_range, factor):
    for i in range(len(points_range)):
        if points_range[i][0] <= factor < points_range[i][1]:
            return points_range[i][2]
    return 0


# similar as above but with one more flag and its related point, if the flag is true, will add its related point
def getRangeWithFlagPoint(
    points_range, factor, flag: bool = False, with_flag_point: int = 0
):
    basic_point = getRangePoint(points_range, factor)
    if basic_point == 0:
        return 0

    return basic_point + with_flag_point if flag else basic_point
